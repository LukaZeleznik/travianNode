<template>
    <div>             
        <h5 class="mt-5"> <p>Current capacity: {{ buildingInfoLookup[$parent.villageBuildingType]['buildingModifier'][$parent.villageBuildingLevel] * config.STORAGE_MODIFIER }} </p></h5>
        <div v-if="$parent.villageBuildingLevel < (buildingInfoLookup[$parent.villageBuildingType]['wood'].length-1)">
            <h5> <p>Capacity at level {{ $parent.villageBuildingLevel+1 }}: {{ buildingInfoLookup[$parent.villageBuildingType]['buildingModifier'][$parent.villageBuildingLevel+1] * config.STORAGE_MODIFIER }}</p></h5>
